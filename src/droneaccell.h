...
uint8_t Drone_accell::decoder(uint8_t x)
{
    return (x ^ 0x17) + 0x17;
}

uint8_t Drone_accell::read_data()
{
    uint8_t i;
...
uint8_t Drone_accell::accell_x()
{
    return ((uint16_t) buffer[2] << 2) | ((buffer[5] >> 2) & 3);
}

uint8_t Drone_accell::accell_y()
{
    return ((uint16_t) buffer[3] << 2) | ((buffer[5] >> 4) & 3);
}

uint8_t Drone_accell::accell_z()
{
